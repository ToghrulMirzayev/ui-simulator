...
function drop(event) {
  ...
  if (dropTarget !== draggedItem && container.contains(dropTarget) && !dropTarget.contains(draggedItem)) {
    const offsetX = event.clientX - container.getBoundingClientRect().left - (draggedItem.clientWidth / 2) + (container.clientWidth / 2);
    const offsetY = event.clientY - container.getBoundingClientRect().top - (draggedItem.clientHeight / 2) + (container.clientHeight / 4);
